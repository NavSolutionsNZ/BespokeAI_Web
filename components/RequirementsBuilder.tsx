...
const STATUS_PIPELINE = [
  ...
  { key: 'approved', label: 'Approved' }, { key: 'in_development', label: 'In Development' },
...
function statusLabel(s: string) { return s === 'needs_clarification' ? 'Needs Clarification' : STATUS_PIPELINE.find(p => p.key === s)?.label ?? s.replace(/_/g,' ') }
...
export default function RequirementsBuilder({ userRole, tenantId }: Props) {
  ...
  const [showAnswers, setShowA] = useState(false)
  const [answerDraft, setAD]    = useState('')
  // Actions
  ...
  function selectReq(req: Requirement) {
    setSelected(req); setShowCreate(false); setShowA(false)
    setAD(req.customerAnswers ?? ''); setAAD(''); setShowSB(false); setShowQF(false); setSpecErr('')
  }
  ...
  async function generateSpec(req: Requirement, answers?: string) {
    setGen(true); setSpecErr('')
    try {
      const res = await fetch(`/api/requirements/${req.id}/ai-spec`, {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ customerAnswers: answers ?? answerDraft })
      })
      const d = await res.json()
      if (!res.ok) throw new Error(d.error)
  ...
  return (
    <div style={{ flex:1, display:'flex', overflow:'hidden' }}>
      ...
      <div style={{ width: panelOpen ? 360 : '100%', flexShrink:0, display:'flex', flexDirection:'column', borderRight: panelOpen ? '1px solid var(--fog)' : 'none', overflow:'hidden', transition:'width 0.2s' }}>
        ...
        {/* Stats */}
        <div style={{ padding:'7px 14px', borderBottom:'1px solid var(--fog)', display:'flex', gap:12, background:'var(--cream)', alignItems:'center' }}>
          {[['Total', reqs.length],['Active', reqs.filter(r=>['submitted','needs_clarification','in_review','quoted','approved','in_development'].includes(r.status)).length],['Done', reqs.filter(r=>r.status==='complete').length]].map(([l,c]) => (
          ...
          {needsClarifCount > 0 && !isSuperadmin && (
            <span style={{ marginLeft:'auto', fontFamily:'var(--font-mono)', fontSize:8, letterSpacing:'0.1em', textTransform:'uppercase', color:'#A32D2D', background:'rgba(163,45,45,0.07)', border:'1px solid rgba(163,45,45,0.2)', padding:'2px 8px', borderRadius:20 }}>
              ⚠ {needsClarifCount} need{needsClarifCount>1?'':'s'} your response
            </span>
          )}
        </div>
        {/* List */}
        <div style={{ flex:1, overflowY:'auto', padding:'10px 10px' }}>
          ...
          {filtered.map(req => {
            ...
            const needsAction = req.status === 'needs_clarification' && !isSuperadmin
            ...
            return (
              <div key={req.id} onClick={() => selectReq(req)} style={{ background: needsAction?'rgba(163,45,45,0.03)':isAct?'rgba(10,92,70,0.05)':'var(--white)', border:`1px solid ${needsAction?'rgba(163,45,45,0.2)':isAct?'rgba(10,92,70,0.22)':'var(--fog)'}`, borderRadius:9, padding:'11px 13px', marginBottom:7, cursor:'pointer', transition:'border-color 0.15s' }}>
                ...
                <div style={{ display:'flex', gap:6, flexWrap:'wrap', alignItems:'center' }}>
                  ...
                  {spec?.questions?.length ? <span style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'#C8952A' }}>? {spec.questions.length}q</span> : null}
        ...
        <div style={{ flex:1, overflowY:'auto', background:'var(--cream)', padding:'22px 26px', display:'flex', flexDirection:'column', gap:18 }}>
          ...
          {showCreate && <>
            ...
            <div style={crd}>
              ...
              <p style={{ fontFamily:'var(--font-body)', fontSize:11, color:'var(--slate)', marginBottom:10, lineHeight:1.55 }}>
                Write in plain English — describe the business problem, who is involved, current workarounds, and what success looks like. The AI will generate a spec and ask clarifying questions. You can refine before submitting.
              </p>
          ...
          {selected && !showCreate && (() => {
            ...
            return <>
              ...
              {/* Needs clarification banner — customer */}
              ...
              {/* Pipeline */}
              {!['needs_clarification','rejected'].includes(req.status) && (
                <div style={crd}>
              ...
              <div style={crd}>
                ...
                {req.customerAnswers && (
                  <div style={{ marginTop:12, paddingTop:12, borderTop:'1px solid var(--fog)' }}>
                    <label style={{ ...lbl, color:'var(--jade)' }}>Additional context provided</label>
                    <p style={{ fontFamily:'var(--font-body)', fontSize:12, color:'var(--slate)', lineHeight:1.7, whiteSpace:'pre-wrap' }}>{req.customerAnswers}</p>
                  </div>
                )}
              </div>
              ...
              {spec ? (
                <div style={{ ...crd, padding:'18px 20px' }}>
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
                    ...
                    {(req.status==='draft'||req.status==='needs_clarification'||isSuperadmin) && (
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
                    ...
                    {spec.assumptions?.length>0 && (
                      <Sect title="Assumptions Made" titleColor="#C8952A">
                        ...
                        <p style={{ fontFamily:'var(--font-body)', fontSize:11, color:'var(--slate)', fontStyle:'italic' }}>If any of these are wrong, answer the questions below and regenerate.</p>
                      </Sect>
                    )}
                      ...
                      <div style={{ background:'rgba(200,149,42,0.06)', border:'1px solid rgba(200,149,42,0.25)', borderRadius:8, padding:'14px 16px' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10 }}>
                          ...
                          <span style={{ fontFamily:'var(--font-mono)', fontSize:9, letterSpacing:'0.12em', textTransform:'uppercase', color:'#9A6A00', fontWeight:600 }}>
                            Clarifying Questions — answer these to refine the spec
                          </span>
                        </div>
                        ...
                        {(req.status==='draft'||req.status==='needs_clarification'||isSuperadmin) && (
                          showAnswers ? (
                            <div>
                              <textarea
                                placeholder={'Answer each question in order, e.g.\n1. Yes, approval required for all orders above $2,000.\n2. Approvers are department managers in BC user setup...'}
                                value={answerDraft}
                              ...
                              <div style={{ display:'flex', gap:8 }}>
                                <button onClick={() => generateSpec(req, answerDraft)} disabled={genSpec||!answerDraft.trim()} style={{ background:'#0A5C46', color:'var(--white)', border:'none', borderRadius:8, padding:'9px 18px', cursor:'pointer', fontFamily:'var(--font-body)', fontSize:13, fontWeight:500, opacity:!answerDraft.trim()?0.6:1 }}>
                                  {genSpec ? '✦ Regenerating…' : '✦ Regenerate with Answers'}
                                </button>
                                <button onClick={() => setShowA(false)} style={sBTN}>Cancel</button>
                              </div>
                              {specErr && <p style={{ color:'#A32D2D', fontSize:11, marginTop:8 }}>{specErr}</p>}
                            </div>
                          ) : (
                            <button onClick={() => setShowA(true)} style={{ background:'rgba(200,149,42,0.12)', border:'1px solid rgba(200,149,42,0.3)', color:'#7A5200', borderRadius:8, padding:'8px 16px', cursor:'pointer', fontFamily:'var(--font-body)', fontSize:12, fontWeight:500 }}>
                ...
                <div style={{ ...crd, textAlign:'center', padding:'22px 20px' }}>
                  ...
                  <p style={{ fontFamily:'var(--font-body)', fontSize:13, color:'var(--slate)', marginBottom:14, lineHeight:1.65 }}>
                    Generate an AI spec. Our assistant will act as a senior BC consultant and developer — producing a user story, acceptance criteria, the exact BC objects that would be modified, key assumptions, and clarifying questions to refine the scope before development starts.
                  </p>
              ...
              {showQF && isSuperadmin && (
                <div style={{ ...crd, borderColor:'rgba(10,92,70,0.25)', background:'rgba(10,92,70,0.03)' }}>
                  <label style={lbl}>Add Quote</label>
                  ...
                  <textarea placeholder="e.g. Includes design, development and testing. Deployment is a separate fixed fee." value={quoteNote} onChange={e => setQN(e.target.value)} rows={3} style={{ ...iSt, resize:'vertical', marginBottom:12 }} onFocus={fo} onBlur={bl} />
                  <div style={{ display:'flex', gap:8 }}>
